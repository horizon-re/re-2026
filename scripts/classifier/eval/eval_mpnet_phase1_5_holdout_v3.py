...
ABLATION_ROOT = Path("classifier/models/mpnet_phase1_5_ablation")
...
def main():
    ...
    for n in window_sizes:
        ...
        # Try to load trained model for this window size
        model_found = False
        model_type = "no_model_found"
        
        # Look for trained ablation model with n{n}-{n}_full pattern
        if ABLATION_ROOT.exists():
            pattern = f"n{n}-{n}_full"
            matching_runs = [d for d in ABLATION_ROOT.iterdir() 
            ...
            if matching_runs:
                ...
                if ckpt_path.exists():
                    print(f"  Loading trained model: {latest_run.name}")
                    ckpt = torch.load(ckpt_path, map_location=DEVICE)
                    head_n = AdaptiveHead(ckpt["head_state"]).to(DEVICE)
                    head_n.eval()
                    
                    with torch.no_grad():
                        probs = torch.sigmoid(head_n(X.to(DEVICE))).cpu().numpy()
                    
                    model_found = True
                    model_type = f"trained_ablation_{latest_run.name}"
        
        # Fallback: use original Phase-1.5 if n=2 and no ablation model found
        if not model_found and n == 2:
            print(f"  Using original Phase-1.5 model")
            with torch.no_grad():
                probs = torch.sigmoid(h15_original(X.to(DEVICE))).cpu().numpy()
            model_type = "original_phase1.5"
            model_found = True
        
        # If still no model, skip this configuration
        if not model_found:
            print(f"  ⚠ No trained model found for n={n}, skipping...")
            continue
